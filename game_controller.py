...
class GameController:
    ...
    def submit_answer(self, answer):
        ...
        # NOTE: daily mode pulls questions at "intermediate" difficulty (see
        # start_session), but self.difficulty is "daily" — saving self.difficulty
        # here mixes up per-difficulty stats. Save the actual question difficulty
        # that was used to select this round instead.
        saved_difficulty = "intermediate" if self.is_daily else self.difficulty
        db.save_session(self.user_id, atk["attack_type"], result_str,
                        pts, round(elapsed, 1), saved_difficulty)
